package frc.robot.commands.Arm;

import edu.wpi.first.math.controller.PIDController;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;
import edu.wpi.first.wpilibj2.command.CommandBase;
import frc.robot.subsystems.ArmSubsystem;

public class resetAxisHall extends CommandBase {
  ArmSubsystem m_arm;
  double motor1output;
  double motor2output;
  double motor3output;

  double axis1;
  double axis2;
  double axis3;

  PIDController controller2 = new PIDController(0.0055, 0, 0);
  PIDController controller3 = new PIDController(0.00415, 0, 0);

  public resetAxisHall(ArmSubsystem m_arm) {
    this.m_arm = m_arm;

    m_arm.Axis1MotorBreakMode();
    m_arm.Axis2MotorBreakMode();
    m_arm.Axis3MotorBreakMode();

    addRequirements(m_arm);
  }

  @Override
  public void initialize() {
  }

  @Override
  public void execute() {

    controller2.setSetpoint(0);
    controller3.setSetpoint(0);

    motor2output = controller2.calculate(-m_arm.getOutputAngle_Axis2);
    motor3output = controller3.calculate(-m_arm.getOutputAngle_Axis3);

    SmartDashboard.putNumber("axis3value1234", motor3output);

    if (0 > m_arm.getOutputAngle_Axis2) {
      m_arm.Axis2MotorOutput((Math.min(-motor2output, 0.20) * 1.6));
    } else if (m_arm.getOutputAngle_Axis2 > 0) {
      m_arm.Axis2MotorOutput((Math.max(-motor2output, -0.20) * 1.6));
    }

    if (0 > m_arm.getOutputAngle_Axis3) {
      m_arm.Axis3MotorOutput(Math.max(-motor3output, -0.25));

    } else if (m_arm.getOutputAngle_Axis3 > 0) {
      m_arm.Axis3MotorOutput(Math.min(-motor3output, 0.25));
    }
  }

  @Override
  public void end(boolean interrupted) {
  }

  @Override
  public boolean isFinished() {
    return (m_arm.getAxis2Hall()) && (m_arm.getAxis3Hall());
  }
}
